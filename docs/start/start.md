...
- While some modules are also built for `arm64` architectures (e.g. used in *NVIDIA Jetson Orin*), the complete toolchain is only supported on `amd64`.
...
- An NVIDIA GPU with at least 12 GB of VRAM and compute capability 8.0 or higher is required.
...
Clone the [OpenADSim](https://github.com/openads-project/openadsim) repository and start the provided docker composition. This will pull the required Docker images and run the stack in a CARLA simulation.
